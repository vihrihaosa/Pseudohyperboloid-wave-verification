...
Версия v47.1 UNIVERSAL FIELD DIAGNOSTICS / CENTER-SHIFTED R2 WINDOW:
  ...
  - v47.3: исправлена физическая ширина выходного окна: по умолчанию материал-функция
    больше НЕ расширяет R2 по радиусу на ±0.5*wall_t. Реальная открытая
    радиальная полоса теперь равна M1=[R+offset, R+offset+R2] в lower_edge
    режиме или [R+shift-W/2,R+shift+W/2] в center режиме. Дополнительный
    зазор можно включить только явно через --aperture-r-clearance;
  - v47.3: добавлены audit-метрики physical_window_* и M1_equals_physical_window;
  - v47.4-control: добавлен контрольный параметр --horn-model straight:
    наружные гиперболические PHB-сегменты заменяются прямыми образующими
    r=R*(L-|z|)/(L-a), а центральное закрытие open/cylinder/halfring,
    R2-окно и мониторы остаются прежними;
  - v47.3: source-near threshold для классификации пиков теперь берётся из
    --source-exclusion-cells, и добавлена nearest-source диагностика для
    E2_peak_excl_sources;
...
Запуск из WSL-папки /home/qwerty/meep-working:
  ...
  docker run --rm -it -v "$PWD":/work -e MPLBACKEND=Agg meep-working \
    python /work/PHB_v47_3_UNIVERSAL_FIELD_DIAGNOSTICS_STRICT_APERTURE.py --stage plan
...
class PHBGeometry:
    ...
    horn_model: str = "hyperbolic"  # "hyperbolic" original PHB horns; "straight" control linear horns
    ...
    def rho_horn(self, abs_z: float) -> float:
        ...
        ``horn_model='hyperbolic'`` is the original PHB law:
            rho = R - b*sqrt((|z|/a)^2 - 1).

        ``horn_model='straight'`` is a deliberately non-PHB control geometry:
            rho_line = R*(L-|z|)/(L-a),  a <= |z| <= L.
        It keeps the same a,b,R,L and the same central closure type, but replaces
        only the two outer hyperbolic segments by straight conical generators.
        """
        if abs_z < self.a:
            return float("nan")
        model = str(getattr(self, "horn_model", "hyperbolic")).lower()
        if model in ("straight", "linear", "line", "cone", "conical", "straight_horns"):
            denom = max(self.L - self.a, 1e-300)
            val = self.R * (self.L - float(abs_z)) / denom
            return max(0.0, val)
    ...
    def z_for_rho(self, rho: float) -> float:
        ...
        rho = float(rho)
        if model in ("straight", "linear", "line", "cone", "conical", "straight_horns"):
            # Inverse of rho_line = R*(L-z)/(L-a), z>=a.
            return self.L - (max(0.0, min(self.R, rho)) / max(self.R, 1e-300)) * (self.L - self.a)
...
def validate_case(g: PHBGeometry, cfg: RunConfig) -> Dict[str, object]:
    ...
    phys_r_min, phys_r_max, phys_w = physical_window_interval(g, cfg)
    physical_radial_cells = phys_w * cfg.resolution
    ...
    horn_model = str(getattr(g, "horn_model", "hyperbolic")).lower()
    if horn_model not in ("hyperbolic", "straight", "linear", "line", "cone", "conical", "straight_horns"):
        problems.append(f"unknown horn_model={horn_model!r}; use hyperbolic or straight")
    if horn_model in ("straight", "linear", "line", "cone", "conical", "straight_horns"):
        warnings.append(
            "CONTROL GEOMETRY: outer PHB hyperbolic horns are replaced by straight conical generators; "
            "central closure, R2 aperture and monitors are unchanged."
        )

    if getattr(cfg, "window_model", "vertical_shift") == "vertical_shift":
        if aperture_r_clearance(cfg) > 0.0:
            warnings.append(
                f"STRICT-APERTURE OVERRIDE: --aperture-r-clearance={aperture_r_clearance(cfg):g}; "
                f"physical window is wider than M1/R2 by {2*aperture_r_clearance(cfg):g}."
            )
        else:
            warnings.append(
                "strict vertical_shift aperture: physical R2 window equals nominal M1 radial interval; "
                "no hidden ±wall_t radial widening is applied"
            )
    ...
    return {
        ...
        "physical_window_r_min": phys_r_min,
        "physical_window_r_max": phys_r_max,
        "physical_window_width": phys_w,
        "physical_window_radial_cells": physical_radial_cells,
        "aperture_r_clearance": aperture_r_clearance(cfg),
        "M1_equals_physical_window": bool(abs(phys_r_min - g.window_r_low(cfg.phb_type)) <= 1e-12 and abs(phys_r_max - g.window_r_high(cfg.phb_type)) <= 1e-12),
        "window_model": getattr(cfg, "window_model", "vertical_shift"),
        "horn_model": str(getattr(g, "horn_model", "hyperbolic")),
...
def plot_geometry(out_png: Path, g: PHBGeometry, cfg: RunConfig) -> None:
    ...
    ax.set_title(
        f"PHB v46 geometry: type={cfg.phb_type}, horn={getattr(g, 'horn_model', 'hyperbolic')}, "
        f"a={g.a:g}, b={g.b:g}, R={g.R:g}, R2=W={g.R2:g}, "
    ...
    note = (
        f"horn model={getattr(g, 'horn_model', 'hyperbolic')}; window model={getattr(cfg, 'window_model', 'vertical_shift')}; z=[{z0:.4g},{z1_eff:.4g}], widened={widened}\n"
...
def aperture_r_clearance(cfg: RunConfig) -> float:
    """Explicit radial clearance added to the physical aperture.

    v47.3 default is 0.0.  Earlier versions effectively used ~0.5*wall_t,
    which made the physical opening much wider than the nominal M1/R2 monitor.
    """
    return max(0.0, float(getattr(cfg, "aperture_r_clearance", 0.0)))


def physical_window_interval(g: PHBGeometry, cfg: RunConfig) -> Tuple[float, float, float]:
    c = aperture_r_clearance(cfg)
    r0 = max(0.0, float(g.window_r_low(cfg.phb_type)) - c)
    r1 = float(g.window_r_high(cfg.phb_type)) + c
    return r0, r1, max(0.0, r1 - r0)
...
def make_material_function(g: PHBGeometry, cfg: RunConfig):
    ...
    def matfun(pos):
        ...
        if getattr(cfg, "window_model", "vertical_shift") == "vertical_shift" and g.R2 > 0.0 and z > 0.0:
            r0_win, r1_win, _w_phys = physical_window_interval(g, cfg)
            if z0_slot <= z <= z1_slot and r0_win <= r <= r1_win:
                return air
...
def add_source_proximity_metrics(row: Dict[str, object], source_records: List[Dict[str, object]], cfg: RunConfig) -> None:
    ...
    near_cells = float(getattr(cfg, "source_exclusion_cells", 3.0))
    near_threshold = near_cells * cell
    row["source_near_peak_threshold_cells"] = near_cells
    ...
    # Also classify peaks after source-near-field exclusion.  This catches
    # cases where a too-small exclusion radius merely moves the maximum to the
    # rim of the masked source neighbourhood.
    excl_prefixes: List[str] = []
    for key in row.keys():
        if key.endswith("_E2_peak_excl_sources_r"):
            prefix = key[:-len("_E2_peak_excl_sources_r")]
            if f"{prefix}_E2_peak_excl_sources_z" in row:
                excl_prefixes.append(prefix)
    for prefix in sorted(set(excl_prefixes)):
        pr = safe_float(row.get(f"{prefix}_E2_peak_excl_sources_r"))
        pz = safe_float(row.get(f"{prefix}_E2_peak_excl_sources_z"))
        if not (math.isfinite(pr) and math.isfinite(pz)):
            continue
        best = None
        for rec in source_records:
        ...
        dist, dr, dz, inside, rec = best
        idx = int(safe_float(rec.get("source_index"), -1))
        row[f"{prefix}_E2_peak_excl_sources_nearest_source_index"] = idx
        row[f"{prefix}_E2_peak_excl_sources_nearest_source_component"] = rec.get("component")
        row[f"{prefix}_E2_peak_excl_sources_nearest_source_kind"] = rec.get("source_kind")
        row[f"{prefix}_E2_peak_excl_sources_nearest_source_r"] = safe_float(rec.get("center_r"))
        row[f"{prefix}_E2_peak_excl_sources_nearest_source_z"] = safe_float(rec.get("center_z"))
        row[f"{prefix}_E2_peak_excl_sources_nearest_source_distance_rz"] = float(dist)
        row[f"{prefix}_E2_peak_excl_sources_nearest_source_distance_cells"] = float(dist / max(cell, 1e-300))
        row[f"{prefix}_E2_peak_excl_sources_inside_source_extent"] = bool(inside)
        row[f"{prefix}_E2_peak_excl_sources_likely_source_artifact"] = bool(inside or dist <= near_threshold)
...
def case_name(phb_type: str, m: int, R2: float, resolution: int, aol: float, horn_model: str = "hyperbolic") -> str:
    def f(x: float) -> str:
        return (f"{x:.6g}".replace("-", "m").replace(".", "p"))
    h = str(horn_model).lower()
    htag = "" if h in ("", "hyperbolic") else f"_horn_{h.replace('-', '_')}"
    return f"{phb_type}{htag}_m{m}_R2_{f(R2)}_res{resolution}_aol_{f(aol)}"


def run_one_case(args, phb_type: str, m: int, R2: float) -> Dict[str, object]:
    g = PHBGeometry(a=args.a, b=args.b, R=args.R, R2=R2, window_offset=args.window_offset, window_offset_mode=args.window_offset_mode, horn_model=args.horn_model)
    cfg = RunConfig(
        ...
        aperture_r_clearance=args.aperture_r_clearance,
    ...
    cname = case_name(phb_type, m, R2, args.resolution, args.a_over_lambda, args.horn_model)
    ...
    event_base = {
        "time": now_iso(), "case": cname, "phb_type": phb_type, "horn_model": args.horn_model, "m": m, "R2_abs": R2, "window_offset": args.window_offset, "window_offset_mode": args.window_offset_mode,
    ...
    phys_r_min, phys_r_max, phys_w = physical_window_interval(g, cfg)
    row["physical_window_r_min"] = phys_r_min
    row["physical_window_r_max"] = phys_r_max
    row["physical_window_width"] = phys_w
    row["physical_window_radial_cells"] = phys_w * cfg.resolution
    row["aperture_r_clearance"] = aperture_r_clearance(cfg)
    row["M1_equals_physical_window"] = bool(abs(phys_r_min - g.window_r_low(cfg.phb_type)) <= 1e-12 and abs(phys_r_max - g.window_r_high(cfg.phb_type)) <= 1e-12)
...
def write_summary(outroot: Path, rows: List[Dict[str, object]], args) -> None:
    ...
    primary = [
        ...
        "M1_r_min", "M1_r_max", "M1_r_center", "physical_window_r_min", "physical_window_r_max", "physical_window_width", "physical_window_radial_cells", "aperture_r_clearance", "M1_equals_physical_window", "M1_focal_axis_r", "M1_lower_edge_on_focal_axis", "M1_center_offset_from_focal_axis", "M1_is_shifted_above_focal_axis", "M1_is_symmetric_about_focal_axis",
...
def write_plan(args) -> None:
    ...
    g0 = PHBGeometry(a=args.a, b=args.b, R=args.R, R2=parse_float_list(args.R2_list)[0], horn_model=args.horn_model)
    ...
    lines.append(f"  horn_model={args.horn_model} (hyperbolic=original PHB; straight=linear/conical control horns)")
...
def run_grid(args) -> int:
    ...
    for phb_type in phb_types:
        for R2 in R2s:
            ...
            htag = "" if str(args.horn_model).lower() == "hyperbolic" else f"_horn_{str(args.horn_model).lower().replace('-', '_')}"
            plot_geometry(outroot / "geometry" / f"geometry_{phb_type}{htag}_R2_{str(R2).replace('.', 'p')}.png", g, cfg)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--horn-model", choices=["hyperbolic", "straight"], default="hyperbolic",
                    help="outer horn generator: hyperbolic = original PHB; straight = control geometry with linear/conical outer segments, same L and same closure")
    ...
    ap.add_argument("--aperture-r-clearance", type=float, default=0.0,
                    help="explicit radial widening added to physical R2 aperture on each side. v47.3 default is 0.0: physical window equals nominal M1/R2 exactly")
    ...
    ap.add_argument("--source-exclusion-cells", type=float, default=8.0,
                    help="radius around point sources, in grid cells, masked in source-excluded |E|² maps; v47.3 default 8 cells for conservative point-source near-field removal")
